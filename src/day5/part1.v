...
fn main() {
	...
	_, _, max_x, max_y := find_coordinate_plane(straight_coord_pairs)
	// create coordinate matrix
	mut plane := generate_coord_plane(max_x, max_y)
	// fill in with coverred points
	plane = fill_plane_from_pairs(plane, straight_coord_pairs)
	// count points >= 2
...
fn fill_plane_from_line(plane [][]int, coord_pair []Coordinate) [][]int {
...
fn fill_plane_from_pairs(plane [][]int, coord_pairs [][]Coordinate) [][]int {
	mut new_plane := plane.clone()
	for pair in coord_pairs {
		new_plane = fill_plane_from_line(new_plane, pair)
	}
	return new_plane
}
...
fn test() {
	...
	assert fill_plane_from_line([[0, 0, 0], [0, 0, 0]], [Coordinate{0, 0},
		Coordinate{0, 1}]) == [[1, 1, 0], [0, 0, 0]]

	assert fill_plane_from_pairs([[0, 0, 0], [0, 0, 0]], [
